...
public class KafkaConsumerSample {
	...
	public static void main(String[] args) {
		...
		props.put("group.id", "group"); // 需要服务器端先配置／创建？ 不需要
		props.put("enable.auto.commit", "false"); // 关闭自动commit
		props.put("session.timeout.ms", "30000");
		props.put("auto.offset.reset", "earliest"); // 初次连接上从哪里开始取数据
		props.put("key.deserializer", "org.apache.kafka.common.serialization.StringDeserializer");
		props.put("value.deserializer", "org.apache.kafka.common.serialization.StringDeserializer");
		KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
		consumer.subscribe(Arrays.asList("message_public", "test2")); // 可以订阅多个topic
